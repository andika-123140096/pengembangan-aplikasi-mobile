...
- **DI layer**: Koin (`dataModule`, `viewModelModule`, & `platformModule`)

## Fitur Implementasi Pertemuan 10 (Testing & Advanced DI)

1. **Refaktor Koin DI**: Memisahkan modul menjadi `dataModule` (untuk repository dan provider) serta `viewModelModule` untuk manajemen dependency yang lebih modular.
2. **Repository Unit Testing**: Implementasi unit test untuk `NotesRepositoryImpl` menggunakan MockK untuk mensimulasi query database SQLDelight (5 test cases).
3. **ViewModel Testing dengan MockK & Turbine**: 
    - Unit test untuk logika bisnis ViewModel (minimal 4 test cases).
    - Flow testing menggunakan Turbine untuk memvalidasi emisi `uiState` dan fitur pencarian (minimal 2 test cases).
4. **Compose UI Testing**: Pengujian antarmuka pengguna pada `NotesScreen` untuk memastikan elemen UI seperti FAB dan daftar catatan ditampilkan dengan benar (minimal 3 test cases).
5. **Coverage Business Logic**: Memastikan cakupan testing minimal 60% untuk komponen inti aplikasi.
- **Platform wiring**:
